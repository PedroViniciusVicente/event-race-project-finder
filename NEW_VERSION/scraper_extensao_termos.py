...
LANGUAGE          = "JavaScript"
MIN_STARS         = 20             # applied post-fetch
...
def passes_deep_filter(diff_content: str) -> tuple[list[str], list[str]]:
    """
    ...
    Returns
    -------
    tuple[list[str], list[str]] — (diff_test_terms, diff_async_terms).
    Both lists must be non-empty for the PR to pass.
    """
    if not diff_content:
        return [], []

    lower = diff_content.lower()
    diff_test_terms = [term for term in TEST_TERMS if term.lower() in lower]
    diff_async_terms = [term for term in ASYNC_TERMS if term.lower() in lower]
    return diff_test_terms, diff_async_terms
...
def get_repo_info(session: requests.Session, repo_full_name: str) -> dict:
    ...
    Used by main() to apply the MIN_STARS filter and to populate
...
CSV_HEADERS = [
    ...
    "diff_test_terms",  # 9. Test terms matched in diff (pipe-separated)
    "diff_async_terms", # 10. Async terms matched in diff (pipe-separated)
]
    ...
    for idx, (d_start, d_end) in enumerate(intervals, start=1):
        ...
        for keyword in KEYWORDS:
            ...
            for pr in raw_items:
                ...
                diff_test_terms, diff_async_terms = passes_deep_filter(diff_content)

                if not diff_test_terms:
                    stats["no_tests"] += 1
                    log.debug("    ✗ No test terms in diff — skipping.")
                    continue

                # ── Deep filter: async terms ──────────────────────────────
                if not diff_async_terms:
                    stats["no_async"] += 1
                ...
                record = {
                    ...
                    "diff_test_terms":  " | ".join(diff_test_terms),
                    "diff_async_terms": " | ".join(diff_async_terms),
                }
