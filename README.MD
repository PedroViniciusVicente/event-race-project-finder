...
In this repository, JavaScript projects containing event races, a special form of race condition, were searched, filtered and analaysed using Pull Requests (PRs) collected from the GitHub API. The 20 analysed projects described in the shortpaper were also classified according to its caracteristics and reproduced following a protocol to reveal flaky tests.
...
We have already included the [final, filtered Pull Requests dataset](data_repos/filtered_event_races_pull_requests.csv) in this repository. However, if you wish to re-run the entire data collection and filtering process, follow these steps.

1. The initial list of JavaScript projects was sourced from the curated lists detailed in the [awesome nodejs lists](awesomeLists/README.md).
...
3. The [filter_search.py](src/github_searches/filter_search2.py) script was used to filter all the collected data, filtering for PRs that match our criteria (mention keywords: "event race", "race condition", "concurrency bug", "race bug", "flaky test"; modify test files with keywords: "test", "it", "describe"; and modify JavaScript files with modern resources: "promises", "async", "await").


### Flaky Tests Reproduction

Our analysis resulted in a dataset of 20 curated bugs that were manually analysed.
